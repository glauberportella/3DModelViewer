...
//    vec4 FragPosLightSpacePoint[NR_POINT_LIGHTS];
//} vsVertex;
//
//
//
...
//      // specular shading
//      vec3 reflectDir = reflect(-lightDir, normal);
//      float spec = 1.0f;//pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
//      // combine results
//      vec3 ambient  = vec3(1.0);//light.ambient  * vec3(texture(material.diffuse, vsVertex.TexCoords));
//      vec3 diffuse  = vec3(1.0);//light.diffuse  * diff * vec3(texture(material.diffuse, vsVertex.TexCoords));
//      vec3 specular = vec3(spec);//light.specular * spec;// * vec3(texture(material.specular, vsVertex.TexCoords));
//
//
//      float shadow = 0;
//      if (shadowsEnabled) {
//        //shadow = ShadowCalculation(vsVertex.FragPosLightSpaceDir, light.shadowMap, normal, lightDir);
//      }
//
//       return ((1.0 - shadow) * (ambient + diffuse + specular));
//////     if (shadow > 0.5) {
...
//    }
//
//    vec3 ambient  = light.ambient  * vec3(texture(material.diffuse, vsVertex.TexCoords));
//    vec3 diffuse  = light.diffuse * diff * vec3(texture(material.diffuse, vsVertex.TexCoords));
//    vec3 specular = light.specular * spec * vec3(texture(material.specular, vsVertex.TexCoords));
//
//    ambient  *= attenuation;
//    diffuse  *= attenuation;
//    specular *= attenuation;
...
//void main(void) {
//    // properties
//    vec3 norm = normalize(vsVertex.Normal);
//    vec3 viewDir = normalize(viewPos - vsVertex.FragPos);
//
//    // phase 1: Directional lighting
//    vec3 result = vec3(0,0,0);
//    if (dirLight.enabled) {
...
////    for(int i = 0; i < NR_POINT_LIGHTS; i++) {
////        if (pointLights[i].enabled) {
////            result += CalcPointLight(pointLights[i], norm, vsVertex.FragPos, viewDir, vsVertex.FragPosLightSpacePoint[i]);
////        }
////    }
////    // phase 3: Spot light
//    //result += CalcSpotLight(spotLight, norm, FragPos, viewDir);
